        ...
        **kwargs,
        ...
        if input_ids.shape[0] != 1:
            raise NotImplementedError(
                "Batches greater than size 1 are not supported.",
            )
        ...
        if self.decode:
            ...
            decoded_output = self.tokenizer.decode(
                input_ids_list[self.input_length :],
            )

            return any(
                stop_sequence in decoded_output
                for stop_sequence in self.stop_sequences
            )
        else:
            ...
            return any(
                any(
                    input_ids_interest[i : i + len(stop_sequence)]
                    == stop_sequence
                    for i in range(
                        len(input_ids_interest) - len(stop_sequence) + 1,
                    )
                )
                for stop_sequence in self.stop_sequences
            )

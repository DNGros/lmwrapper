...
from lmwrapper.huggingface import HuggingfacePredictor
...
def _get_ort_model(model: type[PreTrainedModel]) -> type["ORTModel"]:
    """
    ...
    * Function may warm up models for TensorRT runtime.
    ...
    if runtime == Runtime.PYTORCH:
        ...
        if not is_accelerate_available():
            msg = "Accelerate is not available. Please ensure you have installed `accelerate`."
        ...
        if not Path(save_dir).exists():
            model = _get_ort_model(model_class).from_pretrained(
                pretrained_model_name_or_path=model_name,
                config=model_config,
                export=True,

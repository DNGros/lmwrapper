...
class OpenAiLmPrediction(LmPrediction):
    ...
    def _verify_echo(self):
        if not self.prompt.echo:
            raise ValueError(
                "This property is only available when the prompt `echo` flag"
                " is set",
            )
...
class OpenAIPredictor(LmPredictor):
    ...
    def _validate_prompt(
        ...
        if prompt.logprobs is not None and prompt.logprobs > MAX_LOG_PROB_PARM:
            warnings.warn(
                f"Openai limits logprobs to be <= {MAX_LOG_PROB_PARM}. Larger"
                " values might cause unexpected behavior if you later are"
                " dependingon more returns",
            )
...
def get_open_ai_lm(
    ...
    if api_key_secret is None:
        ...
        if not api_key_secret.is_readable():
            raise ValueError(
                "Cannot find an API key. By default the OPENAI_API_KEY"
                " environment variable is used if it is available. Otherwise it"
                " will read from a file at ~/oai_key.txt. Please place the key"
                " at one of the locations or pass in a SecretInterface (like"
                " SecretEnvVar or SecretFile object) to the api_key_secret"
